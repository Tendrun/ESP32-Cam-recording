...
#pragma region my code
  static esp_err_t recording_handler(httpd_req_t *req) {
      // Set the content type to text/html
      httpd_resp_set_type(req, "text/html");

      // Custom HTML content for the recording page
      const char *html = R"rawliteral(
      <!DOCTYPE html>
      <html>
        <head>
          <title>ESP32-CAM Recording</title>
          <style>
          ...
          <script>
            function startRecording() {
              fetch('/start', { method: 'GET' })
                .then(response => alert('Recording started!'))
                .catch(error => console.error('Error:', error));
            }
            function stopRecording() {
              fetch('/stop', { method: 'GET' })
                .then(response => alert('Recording stopped!'))
                .catch(error => console.error('Error:', error));
            }
          </script>
        </body>
...
httpd_uri_t uri_recording = {
    .uri = "/recording",           // URL path
    .method = HTTP_GET,            // HTTP method
    .handler = recording_handler,  // Handler function
    .user_ctx = NULL               // User context (not used here)
};
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &uri_start);

    httpd_uri_t uri_stop = {"/stop", HTTP_GET, stop_handler, NULL};
    httpd_register_uri_handler(camera_httpd, &uri_stop);
